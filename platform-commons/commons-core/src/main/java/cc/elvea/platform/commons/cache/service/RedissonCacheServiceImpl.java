...
public class RedissonCacheServiceImpl extends AbstractCacheService implements RedissonCacheService {
    ...
    public Set<String> keys(@NonNull String pattern) {
        return Sets.newHashSet(this.redissonClient.getKeys().getKeysByPattern(pattern));
    }
    ...
    public Set<String> scan(@NonNull final String pattern, final int limit) {
        return Sets.newHashSet(this.redissonClient.getKeys().getKeysWithLimit(pattern, limit));
    }
