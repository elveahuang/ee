...
public class SmsAuthenticationProvider extends AbstractAuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        // 检查当前客户端是否包含密码默模式
        if (registeredClient != null && !registeredClient.getAuthorizationGrantTypes().contains(CustomAuthorizationGrantType.OTP)) {
        ...
        // ----- Refresh token -----
        log.info("create refresh token by grant type {}", CustomAuthorizationGrantType.OTP);
        ...
        // ----- ID token -----
        log.info("create id token by grant type {}", CustomAuthorizationGrantType.OTP);
    ...
    private Authentication getSmsAuthentication(SmsAuthenticationToken smsAuthenticationToken) {
        ...
        String mobile = (String) additionalParameters.get(CustomParameterNames.MOBILE_NUMBER);
